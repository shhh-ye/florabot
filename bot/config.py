...
# --- Анти-усыпление (бесплатный план Render) --------------------------------
# Free-план Render усыпляет сервис после 15 минут БЕЗ входящих запросов.
# Разбудить его может входящее сообщение, но холодный старт занимает
# 15-30с, и первый исходящий запрос к Telegram после него часто зависает —
# из-за этого клиент, написавший после паузы, не получает ответа. Чтобы
# сервис не засыпал, он периодически пингует свой собственный публичный
# URL. Render отдаёт его автоматически в RENDER_EXTERNAL_URL; если
# переменной нет (локальный запуск, другой хостинг) — пинг выключен.
SELF_URL = os.environ.get("RENDER_EXTERNAL_URL", "").rstrip("/")
# Как часто пинговать себя, секунды. Заметно меньше 15-минутного порога,
# чтобы таймер простоя не успевал сработать. 0 — выключить пинг совсем.
