...
1. Откройте Render → ваш сервис → **Logs** и найдите строки `SELF-CHECK`
   (появляются через несколько секунд после каждого запуска). Бот сам
   проверяет все свои ключи и токены и пишет по каждому: `OK` — работает,
   `FAIL` — вот точная ошибка. Чините то, что помечено `FAIL`, и делайте
   Manual Deploy (или просто Restart) — проверка прогонится заново.
2. Если все SELF-CHECK зелёные, а бот молчит — смотрите в логах строки
   «Ошибка генерации ответа» (полная причина сбоя на конкретном
   сообщении) и «Ответ отправлен в Telegram» (подтверждение отправки).
3. Частая причина проблем с таблицей — неверно вставленный
   `GOOGLE_CREDENTIALS_JSON` (его нужно вставить как есть, без переносов
   строк, целиком в одну переменную).
