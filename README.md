...
**базе знаний** (RAG: документы → embeddings → векторная база ChromaDB),
...
bot/
  ...
  rag/
    ...
    store.py                — векторная база (ChromaDB) + embeddings (OpenAI)
...
2. Скрипт индексации разбивает их на фрагменты, создаёт для каждого
   embedding (вектор) через OpenAI и сохраняет в ChromaDB.
